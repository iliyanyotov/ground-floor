...
CI (`.github/workflows/ci.yml`) runs exactly `check` → `type-check` → `bun test` on PRs and pushes to `main`. Reproduce CI locally by running those three.
...
- **TC39 decorators** for cross-cutting concerns — `@audit` (`account/audit.ts`) wraps `deposit`/`withdraw` to log after the mutation. It depends on a structural `Auditable` interface rather than importing `Account`, to avoid a cycle.
